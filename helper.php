...
/**
 * Helper for mod_namesonline
 *
 * @since  1.5
 */
class ModNamesonlineHelper
{
	/**
	 * Show online count
	 *
	...
	/**
	 * Show online member full names
	 *
	 * @param   mixed  $params  The parameters
	 *
	 * @return  array   (array) $db->loadObjectList()  The full names of the online users.
	 *
	 * @since   1.5
	 **/
	public static function getOnlineNames($params)
	{
		$whereCondition = JFactory::getConfig()->get('shared_session', '0') ? 'IS NULL' : '= 0';

		$db    = JFactory::getDbo();
		$query = $db->getQuery(true)
			->select($db->quoteName(array('a.username', 'a.userid', 'a.client_id', 'u.name')))
			->from('#__session AS a')
